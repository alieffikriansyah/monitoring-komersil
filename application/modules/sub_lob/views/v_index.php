...
<div class="modal fade" id="m-sub_lob" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<form onsubmit="return SaveGroup(this)">
   <div class="modal-body">  
      ...
      <div class="form-group row">
         ...
         <div class="col-sm-6">
            <input type="text" class="form-control" name="nama_sub_lob" id="nama_sub_lob">
         </div>
      </div>
...
<script>
   ...
   var end = "";
                  ...
                  jQuery.each(json['sub_lob'], function( i, val ) {
                     ...
                     header_table +=`<tr >
                                       ...
                                       <td style="text-align: center;">
                                          <button class="btn waves-effect waves-light btn-primary btn-icon" onclick="EditData(`+val['id_sub_lob']+`)"><i class="feather icon-edit"></i></button>
                                          <button class="btn waves-effect waves-light btn-danger btn-icon" onclick="DeleteData(`+val['id_sub_lob']+`,'delete')"><i class="fa fa-trash"></i></button>
                                       </td>
                                    </tr>`;
                                    no++; 
                  });
    ...
    function AddData(){
      // show();
      $('#m-sub_lob').modal('show');
      $('#m-sub_lob').find('.modal-title').html('Tambah Sub_Lob Baru');   
      $('#m-sub_lob').find('form').attr('onsubmit','return SaveData(this)');
    }
   
   function EditData(id){
      // show();
      $('#m-sub_lob').modal('show');
      $('#m-sub_lob').find('.modal-title').html('Edit sub_lob');   
      $('#m-sub_lob').find('form').attr('onsubmit','return UpdateData(this,\''+id+'\')');
      $.ajax({
               url: "<?=base_url()?>Sub_Lob/EditData/"+id,
               type: 'post',
               // data: formData,
               contentType: false,
               processData: false,
               ...
               success: function(r){
                  ...
                  var json = JSON.parse(r);
                  $('#id_sub_lob').val(json['id_sub_lob']);  
                  $('#nama_sub_lob').val(json['nama_sub_lob']);
   ...
   function DeleteData(id) {
        ...
        $.ajax({
            url: '<?=base_url('Sub_Lob/')?>Delete/' + id,
            type: 'post',
            contentType: false,
            processData: false,
            success: function (r) {
            ...
            error: function (xhr, status, error) {
                ...
                window.location.href = "<?=site_url('Sub_Lob/index')?>";
            }
        });
  ...
  function UpdateData(form, id) {
    ...
    formData.append('id_sub_lob', id);
    
    $.ajax({
        url: "<?=base_url()?>Sub_Lob/Update",
        type: 'POST',
        data: formData,
        contentType: false,
        processData: false,
