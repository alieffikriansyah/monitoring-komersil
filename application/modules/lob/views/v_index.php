...
<div class="modal fade" id="m-lob" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<form onsubmit="return SaveGroup(this)">
   <div class="modal-body">  
      <div class="form-group row">
         <label class="col-sm-2 col-form-label">Nama Lob</label>
         <div class="col-sm-6">
...
<script>
   ...
   var end = "";
                  ...
                  jQuery.each(json['lob'], function( i, val ) {
                     ...
                     header_table +=`<tr >
                                       ...
                                       <td style="text-align: center;">
                                          ...
                                          <button class="btn waves-effect waves-light btn-danger btn-icon" onclick="DeleteData(`+val['id_lob']+`,'delete')"><i class="fa fa-trash"></i></button>
                                       </td>
                                    </tr>`;
                                    no++; 
                  });
    ...
    function AddData(){
      // show();
      $('#m-lob').modal('show');
      $('#m-lob').find('.modal-title').html('Tambah Lob Baru');   
      $('#m-lob').find('form').attr('onsubmit','return SaveData(this)');
    }
   
   function EditData(id){
      // show();
      $('#m-lob').modal('show');
      $('#m-lob').find('.modal-title').html('Edit lob');   
      $('#m-lob').find('form').attr('onsubmit','return UpdateData(this,\''+id+'\')');
      $.ajax({
               url: "<?=base_url()?>Lob/EditData/"+id,
               type: 'post',
               // data: formData,
               contentType: false,
               processData: false,
               ...
               success: function(r){
                  ...
                  var json = JSON.parse(r);
                  $('#id_lob').val(json['id_lob']);  
                  $('#nama_lob').val(json['nama_lob']);
   ...
   function DeleteData(id) {
        ...
        $.ajax({
            ...
            error: function (xhr, status, error) {
                ...
                window.location.href = "<?=site_url('Lob/index')?>";
            }
        });
  ...
  function UpdateData(form, id) {
    ...
    formData.append('id_lob', id);
    
    $.ajax({
        url: "<?=base_url()?>Lob/Update",
        type: 'POST',
        data: formData,
        contentType: false,
        processData: false,
