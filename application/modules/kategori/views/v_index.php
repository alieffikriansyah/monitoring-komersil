                              ...
                              <table class="table table-condensed table-striped table-bordered" id="tabel-data">
                                 <thead class="thead-blue">
                                    <tr>
                                       ...
                                       <th class="cemter-t">Nama Kategori</th>
...
<div class="modal fade" id="m-kategori" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<script>
   ...
   var end = "";
      ...
      function LoadData(){
         ...
         $.ajax({
               url: "<?=base_url()?>Kategori/LoadData",
               type: 'post',
               // data: formData,
               contentType: false,
               processData: false,
               ...
               success: function(r){
                  ...
                  // var pag= "";
                  var no=1;
                  jQuery.each(json['kategori'], function( i, val ) {
                     ...
                     header_table +=`<tr >
                                       ...
                                       <td style="text-align: center;">
                                          <button class="btn waves-effect waves-light btn-primary btn-icon" onclick="EditData(`+val['id_kategori']+`)"><i class="feather icon-edit"></i></button>
                                          <button class="btn waves-effect waves-light btn-danger btn-icon" onclick="DeleteData(`+val['id_kategori']+`,'delete')"><i class="fa fa-trash"></i></button>
                                       </td>
                                    </tr>`;
                                    no++; 
                  });
    ...
    function AddData(){
      // show();
      $('#m-kategori').modal('show');
      $('#m-kategori').find('.modal-title').html('Tambah kategori Baru');   
      $('#m-kategori').find('form').attr('onsubmit','return SaveData(this)');
    }
   
   function EditData(id){
      // show();
      $('#m-kategori').modal('show');
      $('#m-kategori').find('.modal-title').html('Edit kategori');   
      $('#m-kategori').find('form').attr('onsubmit','return UpdateData(this,\''+id+'\')');
      $.ajax({
               url: "<?=base_url()?>Kategori/EditData/"+id,
               type: 'post',
               // data: formData,
               contentType: false,
               processData: false,
   ...
   function SaveData(f) {
      ...
      $.ajax({
         url: '<?= base_url('Kategori/SaveData/') ?>',
         type: 'post',
         data: formData,
         contentType: false,
         processData: false,
